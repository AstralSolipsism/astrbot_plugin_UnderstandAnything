...
def merge(root: Path) -> dict:
    ...
    if not manifest_path.is_file():
        print(
            f"Error: {manifest_path} not found. Run parse-knowledge-base.py first.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    for bf in batch_files:
        ...
        for node in batch.get("nodes", []):
            ...
            if node_type not in VALID_NODE_TYPES:
                print(
                    f"[merge] Warning: Unknown node type '{node.get('type')}' — skipping",
                    file=sys.stderr,
                )
                continue
        ...
        for edge in batch.get("edges", []):
            ...
            if edge_type not in VALID_EDGE_TYPES:
                print(
                    f"[merge] Warning: Unknown edge type '{edge.get('type')}' — "
                    f"mapped to 'related'",
                    file=sys.stderr,
                )
                edge_type = "related"
    ...
    for nid, node in nodes.items():
        if node["type"] in ("entity", "claim") and nid not in child_to_article:
            ...
            )  # "concept-aaak-compression:not-zero-loss"
    ...
    for cat in categories:
        ...
        layer_id = f"layer:{cat_slug}"
        members = list(
            dict.fromkeys(layer_members.get(layer_id, []))
