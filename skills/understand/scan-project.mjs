 ...
 *
 * Usage:
 *   node scan-project.mjs <projectRoot> <outputPath> [graphRoot]
 *
 ...
 */
function hasUserIgnoreFile(projectRoot, graphRoot = projectRoot) {
  return (
    existsSync(join(projectRoot, '.understandignore'))
    || existsSync(join(graphRoot, '.understandignore'))
...
async function main() {
  const [, , projectRoot, outputPath, graphRootArg] = process.argv;
  const graphRoot = graphRootArg || join(projectRoot || '.', '.understand-anything');
  if (!projectRoot || !outputPath) {
    process.stderr.write(
      'Usage: node scan-project.mjs <projectRoot> <outputPath> [graphRoot]\n',
    );
    process.exit(1);
  }
  ...
  const combined = createIgnoreFilter(projectRoot, graphRoot);
  const userIgnoresPresent = hasUserIgnoreFile(projectRoot, graphRoot);
