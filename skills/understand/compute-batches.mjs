 ...
 *
 * Usage:
 *   node compute-batches.mjs <project-root> [--graph-root=<path>] [--changed-files=<path>]
 *
 * Input:  <graph-root>/intermediate/scan-result.json
 * Output: <graph-root>/intermediate/batches.json
 */
...
const RUNTIME_ROOT = resolve(PLUGIN_ROOT, 'understand-anything');
const require = createRequire(resolve(RUNTIME_ROOT, 'package.json'));

let core;
try {
  core = await import(pathToFileURL(require.resolve('@understand-anything/core')).href);
} catch {
  core = await import(pathToFileURL(resolve(RUNTIME_ROOT, 'packages/core/dist/index.js')).href);
}
const { TreeSitterPlugin, PluginRegistry, builtinLanguageConfigs, registerAllParsers } = core;

const { default: Graph } = await import(pathToFileURL(require.resolve('graphology')).href);
const { default: louvain } = await import(
  pathToFileURL(require.resolve('graphology-communities-louvain')).href
);
...
async function main() {
  ...
  if (!projectRoot) {
    process.stderr.write('Usage: node compute-batches.mjs <project-root> [--graph-root=<path>] [--changed-files=<path>]\n');
    process.exit(1);
  }

  let graphRoot = join(projectRoot, '.understand-anything');
  let changedFiles = null;
  for (const arg of process.argv.slice(3)) {
    const graphRootMatch = arg.match(/^--graph-root=(.+)$/);
    if (graphRootMatch) {
      graphRoot = graphRootMatch[1];
      continue;
    }
  ...
  const scanPath = join(graphRoot, 'intermediate', 'scan-result.json');
  ...
  const outPath = join(graphRoot, 'intermediate', 'batches.json');
...
// a symlinked plugin install path. See upstream issue #162.
// ---------------------------------------------------------------------------
function isCliEntry() {
