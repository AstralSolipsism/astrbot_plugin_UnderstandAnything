...
- `检查更新`：只检查源码新鲜度，例如比较已有图谱记录的 commit 和当前 Git HEAD；不会启动分析，也不会改写图谱。
- `更新图谱`：面向已有图谱的正常更新动作。它启动 `understand` 任务但不传 `--full`；有历史 graph/meta 基线时由内置 skill 走增量更新流程。
- `重新分析`：完整重新分析动作，会强制传 `--full`，适合用户明确要重建图谱或补齐完整分析产物时使用。
...
多项目场景下，如果用户没有明确项目且当前会话也没有项目上下文，插件不会猜测。它会返回候选项目并要求用户选择。用户可以通过 `/understand 项目 <项目名>` 切换当前会话项目，也可以在普通聊天里说“之后都看 <项目名>”。跨项目对比不会自动切换当前项目；LLM 会分别对每个被点名项目做状态确认和图谱检索。
