...
import type { GraphEdge, GraphNode, KnowledgeGraph, ProjectMeta } from "@understand-anything/core/types";
import {
  normalizeInventoryPath,
  sourceInventoryEntryMap,
...
type DomainItemKind = "domain" | "flow" | "step" | "cross-domain";
type ProvenanceOwner = DomainIR | FlowIR | StepIR | CrossDomainInteractionIR;
...
function idToken(input: string | undefined, fallbackPrefix: string): string {
...
function uniqueToken(base: string, used: Set<string>): string {
  let candidate = base;
  let index = 2;
...
function nestedDomainMeta(record: Record<string, unknown>): Record<string, unknown> | null {
  return asRecord(record.domainMeta);
}

function stringFromRecordOrMeta(record: Record<string, unknown>, field: string): string | undefined {
  return asString(record[field]) ?? asString(nestedDomainMeta(record)?.[field]);
}

function stringArrayFromRecordOrMeta(record: Record<string, unknown>, field: string): string[] {
  const direct = asStringArray(record[field]);
  if (direct.length > 0) return direct;
  return asStringArray(nestedDomainMeta(record)?.[field]);
}

function evidenceFromRecordOrMeta(record: Record<string, unknown>): string[] {
...
function warnUnexpectedFinalFields(issues: GraphQualityIssue[], record: Record<string, unknown>, path: string): void {
  for (const field of Object.keys(record)) {
    if (!FINAL_GRAPH_FIELDS.has(field) && !FINAL_NODE_FIELDS.has(field)) continue;
    issue(issues, "warning", "domain-ir-final-graph-field-ignored", `DomainAnalysisIR 中包含最终图谱字段，已忽略：${path}.${field}`, { path: `${path}.${field}` });
  }
}

function normalizeStep(raw: unknown, issues: GraphQualityIssue[], path: string): StepIR | null {
...
function normalizeFlow(raw: unknown, issues: GraphQualityIssue[], path: string): FlowIR | null {
  const record = asRecord(raw);
  if (!record) {
  ...
  const steps = Array.isArray(record.steps)
    ? record.steps.map((item, index) => normalizeStep(item, issues, `${path}.steps[${index}]`)).filter((item): item is StepIR => item !== null)
    : [];
  ...
  return {
    ...
    entryType: entryType && VALID_ENTRY_TYPES.has(entryType) ? entryType as FlowIR["entryType"] : undefined,
    sourceNodeIds: stringArrayFromRecordOrMeta(record, "sourceNodeIds"),
    sourceFilePaths: stringArrayFromRecordOrMeta(record, "sourceFilePaths"),
    evidence: evidenceFromRecordOrMeta(record),
    steps,
...
function normalizeDomain(raw: unknown, issues: GraphQualityIssue[], path: string): DomainIR | null {
  const record = asRecord(raw);
  if (!record) {
  ...
  const flows = Array.isArray(record.flows)
    ? record.flows.map((item, index) => normalizeFlow(item, issues, `${path}.flows[${index}]`)).filter((item): item is FlowIR => item !== null)
    : [];
  return {
    id: asString(record.id),
    name,
...
function normalizeCrossDomainInteraction(raw: unknown, issues: GraphQualityIssue[], path: string): CrossDomainInteractionIR | null {
  const record = asRecord(raw);
  if (!record) {
...
export function normalizeDomainAnalysisIR(raw: unknown): { ir: DomainAnalysisIR | null; issues: GraphQualityIssue[] } {
  ...
  const domains = record.domains
    ...
    .filter((item): item is DomainIR => item !== null);
  if (domains.length === 0) {
  ...
  const crossDomainInteractions = Array.isArray(record.crossDomainInteractions)
    ? record.crossDomainInteractions
      ...
      .filter((item): item is CrossDomainInteractionIR => item !== null)
    : [];
  return { ir: { version: asString(record.version) ?? "1.0.0", domains, crossDomainInteractions }, issues };
}

function cleanSourceNodeIds(rawIds: string[] | undefined, nodeIds: Set<string>, issues: GraphQualityIssue[], nodeName: string, nodeId: string, kind: DomainItemKind): string[] {
  const output: string[] = [];
...
function cleanSourceFilePaths(rawPaths: string[] | undefined, inventory: SourceInventory, issues: GraphQualityIssue[], nodeName: string, nodeId: string, kind: DomainItemKind): string[] {
...
function cleanStepFilePath(rawPath: string | undefined, inventory: SourceInventory, issues: GraphQualityIssue[], nodeName: string, nodeId: string): string | undefined {
...
function cleanLineRange(range: [number, number] | undefined, filePath: string | undefined, inventory: SourceInventory, issues: GraphQualityIssue[], nodeName: string, nodeId: string): [number, number] | undefined {
  if (!range || !filePath) return undefined;
...
function cleanProvenance(owner: ProvenanceOwner, options: {
  nodeIds: Set<string>;
  ...
  nodeId: string;
  kind: DomainItemKind;
}): { sourceNodeIds?: string[]; sourceFilePaths?: string[]; evidence?: string[] } {
...
function projectMetaFrom(knowledgeGraph: KnowledgeGraph, meta: Record<string, unknown>): ProjectMeta {
...
function edgeWeight(index: number, total: number): number {
  if (total <= 1) return 1;
  return Math.max(0.1, Math.min(1, Number(((index + 1) / total).toFixed(1))));
}

function findDomainId(reference: string, domainMap: Map<string, string>): string | undefined {
...
export function compileDomainGraph(ir: DomainAnalysisIR, knowledgeGraph: KnowledgeGraph, inventory: SourceInventory, meta: Record<string, unknown>): CompileDomainGraphResult {
  const issues: GraphQualityIssue[] = [];
  const knowledgeNodeIds = new Set(knowledgeGraph.nodes.map((node) => node.id));
  const nodes: GraphNode[] = [];
  const edges: GraphEdge[] = [];
  const usedDomainTokens = new Set<string>();
  ...
  for (const domain of ir.domains) {
    ...
    if (domain.id) domainMap.set(domain.id, domainId);
    ...
    const usedFlowTokens = new Set<string>();
    for (const flow of domain.flows ?? []) {
      ...
      const usedStepTokens = new Set<string>();
...
function nodeToIrBase(node: GraphNode): Omit<DomainIR, "flows"> {
  return {
...
export function domainGraphToIR(graph: KnowledgeGraph): DomainAnalysisIR {
  ...
  const crossDomainInteractions: CrossDomainInteractionIR[] = [];
  ...
  for (const flow of flowNodes) {
    if ([...flowsByDomain.values()].some((items) => items.includes(flow))) continue;
  ...
  for (const step of stepNodes) {
    if ([...stepsByFlow.values()].some((items) => items.includes(step))) continue;
