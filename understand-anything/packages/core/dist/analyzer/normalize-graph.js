 ...
 */
function stripToValidPrefix(id) {
    let remaining = id;
 ...
 */
export function normalizeNodeId(id, node) {
...
export function normalizeComplexity(value) {
    if (typeof value === "string") {
        ...
        if (VALID_COMPLEXITIES.has(lower))
            return lower;
 ...
 *
 * This runs before the base sanitizeGraph/autoFixGraph/normalizeGraph pipeline,
...
export function normalizeBatchOutput(data) {
    ...
    const stepToFlowSlug = new Map();
    const flowNodeNames = new Map();
    ...
    for (const raw of data.edges) {
        ...
        if (!validNodeIds.has(newSource)) {
            ...
            if (validNodeIds.has(normalized))
...
//# sourceMappingURL=normalize-graph.js.map