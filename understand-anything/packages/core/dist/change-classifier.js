 ...
 */
export function classifyUpdate(analysis, totalFilesInGraph, allKnownFiles = []) {
 ...
 */
function detectDirectoryChanges(newFiles, deletedFiles, allKnownFiles) {
    const existingDirs = new Set(allKnownFiles.map((f) => topDirectory(f)).filter(Boolean));
