...
function layerForNode(graph: KnowledgeGraph, nodeId: string): Layer | null {
  return graph.layers.find((layer) => layer.nodeIds.includes(nodeId)) ?? null;
}

function edgeLine(edge: GraphEdge, nodeMap: Map<string, GraphNode>): string {
...
export function buildExplainContext(
  ...
  let targetNode: GraphNode | null =
    nodes.find((node) => node.id === target) ?? null;
...
export function buildDiffContext(
  ...
  return {
    ...
    impactedEdges,
    affectedLayers: graph.layers.filter((layer) =>
      layer.nodeIds.some((id) => impactedIds.has(id)),
    ),
    unmappedFiles,
  };
}

export function buildDiffOverlay(context: DiffContext): {
...
export function buildOnboardingGuide(graph: KnowledgeGraph): string {
  ...
  if (graph.tour.length > 0) {
    ...
    for (const step of [...graph.tour].sort((a, b) => a.order - b.order)) {
      ...
      if (stepNodes.length > 0) lines.push("");
    }
  }
...
function formatContextItems(
  graph: KnowledgeGraph,
  domainGraph: KnowledgeGraph | null | undefined,
  contextItems: AssistantContextItem[],
): string {
  if (contextItems.length === 0) return "No explicit context items were selected.";
  const nodeMap = new Map<
    string,
    { node: GraphNode; graphKind: "knowledge" | "domain" }
  >(graph.nodes.map((node) => [node.id, { node, graphKind: "knowledge" }]));
  ...
  for (const item of contextItems) {
    if (item.type === "node") {
      ...
      const scopePath = node.domainMeta?.scopePath
        ...
        : "";
      lines.push(
        `- ${graphKind} node ${nodeLabel(node)}, id=${node.id}${node.filePath ? `, file=${node.filePath}` : ""}${scopePath}${sourceFiles}${layer ? `, layer=${layer.name}` : ""}`,
      );
      ...
      lines.push(
        `- File: ${item.path}${item.nodeId ? `, nodeId=${item.nodeId}` : ""}${item.graphKind === "domain" ? ", from domain graph" : ""}`,
      );
    } else if (item.type === "code-range") {
      lines.push(
        `- Code range: ${item.path}:${item.startLine}-${item.endLine}${item.nodeId ? `, nodeId=${item.nodeId}` : ""}${item.graphKind === "domain" ? ", from domain graph" : ""}`,
      );
...
function formatNodes(title: string, nodes: GraphNode[], max = 24): string {
  if (nodes.length === 0) return `## ${title}\nNone.`;
  const lines = [`## ${title}`];
  for (const node of nodes.slice(0, max)) {
    lines.push(
      `- ${nodeLabel(node)}${node.filePath ? `, file=${node.filePath}` : ""}: ${node.summary}`,
    );
  }
  if (nodes.length > max) lines.push(`- ${nodes.length - max} more nodes omitted.`);
  return lines.join("\n");
}
...
function formatEdges(
  ...
  const nodeMap = new Map(graph.nodes.map((node) => [node.id, node]));
  const lines = [`## ${title}`];
  for (const edge of edges.slice(0, max)) lines.push(edgeLine(edge, nodeMap));
  if (edges.length > max) lines.push(`- ${edges.length - max} more edges omitted.`);
  return lines.join("\n");
}

function formatSourceSnippets(snippets: SourceSnippet[]): string {
  if (snippets.length === 0) return "No source snippets were attached.";
  const lines: string[] = [];
  for (const snippet of snippets) {
    ...
    lines.push("```");
    if (snippet.truncated) lines.push("This snippet was truncated to fit context limits.");
    lines.push("");
  }
  return lines.join("\n");
}

function latestUserMessage(messages: AssistantPromptMessage[]): string {
...
export function buildAssistantPrompt(options: BuildAssistantPromptOptions): string {
  ...
  const explicitTargets = options.contextItems
    .map((item) => {
      if (item.type === "node") return item.nodeId;
      if ("nodeId" in item && item.nodeId) return item.nodeId;
  ...
  if (options.gitCommitHash) lines.push(`- Commit: ${options.gitCommitHash}`);
  if (options.graphHash) lines.push(`- Graph hash: ${options.graphHash}`);
  lines.push("");
  ...
  if (options.mode === "explain" && explainTargets.length > 0) {
    for (const target of explainTargets.slice(0, 6)) {
      ...
      if (context.layer) lines.push(`Layer: ${context.layer.name}, ${context.layer.description}`);
  ...
  if (options.mode === "diff" && options.diffContext) {
    ...
    for (const file of options.diffContext.changedFiles.slice(0, 50)) lines.push(`- ${file}`);
    ...
    if (options.diffContext.affectedLayers.length > 0) {
      ...
      for (const layer of options.diffContext.affectedLayers) lines.push(`- ${layer.name}: ${layer.description}`);
    }
    if (options.diffContext.unmappedFiles.length > 0) {
      lines.push("## Unmapped Files");
      for (const file of options.diffContext.unmappedFiles) lines.push(`- ${file}`);
    }
  }
