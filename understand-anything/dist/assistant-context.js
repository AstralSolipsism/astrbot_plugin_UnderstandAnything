...
import { buildAssistantPrompt, buildDiffContext, buildDiffOverlay, buildOnboardingGuide, } from "@understand-anything/assistant";
...
function readOptionalGraph(graphRoot, fileName) {
...
function coerceMode(value) {
    if (value === "explain" || value === "diff" || value === "onboarding")
        return value;
    if (value === "onboard")
        return "onboarding";
    return "chat";
...
function cleanContextPath(value) {
    if (typeof value !== "string" || !value.trim())
        return null;
    const text = value.trim();
    if (text.includes("\0") || text.includes("\\"))
        return null;
    if (/^[A-Za-z]:[\\/]/u.test(text) || text.startsWith("/"))
        return null;
...
function sourceInventoryFileSet(graphRoot) {
    const files = new Set();
    const path = resolve(graphRoot, "source-inventory.json");
    if (!existsSync(path))
        return files;
    try {
        const inventory = readJsonFile(path);
        if (!Array.isArray(inventory.entries))
            return files;
        for (const entry of inventory.entries) {
            const record = asRecord(entry);
            if (record?.kind !== "file" && record?.type !== "file")
                continue;
...
function graphNodeByFile(graph, path) {
    return graph?.nodes.find((node) => node.filePath === path && node.type === "file")
        ?? graph?.nodes.find((node) => node.filePath === path);
}
function resolvePathContext(item, path, graph, domainGraph, domainNodeIds) {
    const rawNodeId = typeof item.nodeId === "string" ? item.nodeId : undefined;
    const requestedDomainNode = item.graphKind === "domain" && rawNodeId && domainNodeIds.has(rawNodeId)
    ...
    if (knowledgeNode)
        return { graphKind: "knowledge", nodeId: knowledgeNode.id };
    if (domainNode)
        return { graphKind: "domain", nodeId: domainNode.id };
    return {};
}
...
function readSnippet(projectRoot, item, warnings) {
    if (item.type !== "file" && item.type !== "code-range")
        return null;
    ...
    const requestedEnd = item.type === "code-range" ? Math.max(startLine, item.endLine) : lines.length;
...
function sanitizeContextItems(options) {
    ...
    for (const raw of rawItems.slice(0, MAX_CONTEXT_ITEMS)) {
        ...
        if (item.type === "node" &&
            ...
            const graphKind = domainNodeIds.has(item.nodeId) && item.graphKind === "domain"
                ? "domain"
...
function forbiddenPathPatterns(projectRoot, graphRoot) {
...
function assertNoForbiddenAbsolutePaths(label, value, projectRoot, graphRoot) {
...
function graphSummary(graph, domainGraph) {
    return {
...
export function buildAssistantContextBundle(payload) {
    ...
    const graph = readJsonFile(resolve(graphRoot, "knowledge-graph.json"));
